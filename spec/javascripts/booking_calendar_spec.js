...
describe('Modules.bookingCalendar', function() {
  ...
  describe('useful functions', function() {
    it('`calcStartWeekday` should return the first weekday of February 2017', function() {
      var date = moj.Modules.bookingCalendar.calcStartWeekday(2017, 1);
      expect(i18n.days[date]).toBe('Wednesday');
    });
    it('`splitDateAndSlot` should split the slot into date and time', function() {
      var split = moj.Modules.bookingCalendar.splitDateAndSlot('2017-02-09T14:00/16:00');
      expect(split[0]).toBe('2017-02-09');
      expect(split[1]).toBe('14:00/16:00');
    });
    it('`formatTime` should return the start time', function() {
      var time = moj.Modules.bookingCalendar.formatTime('14:00/16:00');
      expect(time).toBe('14:00');
    });
    it('`formatTimeDuration` should return the duration of the slot', function() {
      var duration = moj.Modules.bookingCalendar.formatTimeDuration('14:00/16:00');
      expect(duration).toBe('2 hrs');
    });
    it('makeDateObj` should', function() {
      var date = moj.Modules.bookingCalendar.makeDateObj('2017-02-09');
      var testDate = new Date('2017-02-09');
      expect(date).toEqual(testDate);
    });
    it('`duration` should return the duration from two date/time objects', function() {
      var start = new Date('2017-02-09T14:00');
      var end = new Date('2017-02-09T16:00');
      var duration = moj.Modules.bookingCalendar.duration(start, end);
      expect(duration).toBe('2 hrs');
    });
  });
});